...
# The script no longer lives inside the repo it checks: it is shared from
# MustardSeedNetworks/.github, so the repo root cannot be derived from
# __file__. It comes from I18N_REPO_ROOT, defaulting to the working directory
# because every entry point runs from the repo root.
ROOT = Path(os.environ.get("I18N_REPO_ROOT", ".")).resolve()
# The ruleset travels with this script, not with the repo being checked.
RULES = Path(__file__).resolve().parent / "semgrep-i18n.yml"
UI_SRC = ROOT / os.environ.get("UI_SRC_DIR", "ui/src")
