...
# dynamic-prefix allow-list) and is passed in through the env vars below.
# Defaults assume the standard Go-embedded layout, so a repo that follows it
...
# The helper scripts travel with this file; the data they read belongs to the
# repo being checked. Resolving them here is what lets the gate live in one
# place while still being runnable from any repo's root.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
export I18N_REPO_ROOT="${I18N_REPO_ROOT:-$PWD}"

# -----------------------------------------------------------------------------
# Config (override via env) — all paths are relative to the repo being checked
# -----------------------------------------------------------------------------
...
check_no_fallback_patterns() {
  ...
  local out
  if ! out=$(python3 "$SCRIPT_DIR/semgrep-i18n.py" 2>&1); then
...
check_hardcoded_jsx() {
  ...
  # the closing `>`, so JSX text on its own line was invisible and roughly half
  # of each repo's hardcoded strings went unreported. check-source.py blanks
  # comments and matches across lines, and this blocks — there is no baseline
  # file because all three repos were cleaned to zero before it was promoted.
  local out
  if ! out=$(python3 "$SCRIPT_DIR/check-source.py" 2>&1); then
...
check_key_usage() {
  ...
  local script="$SCRIPT_DIR/check-keys.py"
  if [ ! -f "$script" ]; then
